document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('../assets/data/students.json')
        ...
        .then(function(data) {
            allStudents = (data || []).map(normalizeStudent);
        ...
        .catch(function(err) {
            ...
            grid.innerHTML = '<div class="col-span-full text-center py-10 text-red-500">Failed to load student data.</div>';
        });
    ...
    function getFilteredStudents() {
        return allStudents.filter(function(s) {
            ...
            var matchesSearch = !q ||
                ...
                (s.achievement || '').toLowerCase().indexOf(q) !== -1;
    ...
    function renderCardView(students) {
        ...
        students.forEach(function(student, index) {
            ...
            var photoHtml = photoSrc
                ? '<div class="relative w-16 h-16 rounded-full">' + photoFallback + '<img src="' + escapeHtml(photoSrc) + '" alt="' + escapeHtml(student.name) + '" loading="lazy" class="absolute inset-0 w-16 h-16 rounded-full object-cover shadow-lg border-2 border-white bg-white" onerror="this.style.display=\'none\'"></div>'
                : photoFallback;

            var achievementHtml = student.achievement
                ? '<div class="mb-3"><p class="text-[0.7rem] font-mono uppercase tracking-wider" style="color:#1652c4">Achievement</p><p class="text-xs mt-0.5 leading-relaxed" style="color:#5b6478">' + escapeHtml(truncateText(student.achievement, 80)) + '</p></div>'
                : '';

            var githubHtml = student.github
                ? '<a href="' + escapeHtml(getExternalHref(student.github, 'github')) + '" target="_blank" rel="noopener" aria-label="GitHub" title="GitHub" class="w-8 h-8 rounded-lg flex items-center justify-center text-xs transition-all" style="background:#f0f4ff;color:#1652c4" onmouseover="this.style.background=\'#1652c4\';this.style.color=\'#fff\'" onmouseout="this.style.background=\'#f0f4ff\';this.style.color=\'#1652c4\'"><i class="fa-brands fa-github"></i></a>'
                : '';

            var linkedinHtml = student.linkedin
                ? '<a href="' + escapeHtml(getExternalHref(student.linkedin, 'linkedin')) + '" target="_blank" rel="noopener" aria-label="LinkedIn" title="LinkedIn" class="w-8 h-8 rounded-lg flex items-center justify-center text-xs transition-all" style="background:#f0f4ff;color:#1652c4" onmouseover="this.style.background=\'#1652c4\';this.style.color=\'#fff\'" onmouseout="this.style.background=\'#f0f4ff\';this.style.color=\'#1652c4\'"><i class="fa-brands fa-linkedin-in"></i></a>'
                : '';
            ...
            card.innerHTML = [
              ...
              '<div class="p-5">',
                ...
                '<p class="text-xs mt-0.5 mb-2 font-mono" style="color:#1652c4">' + escapeHtml(registerNo) + '</p>',
                achievementHtml,
                '<div class="space-y-1 text-[0.75rem] mb-3" style="color:#5b6478">',
    ...
    function normalizeStudent(student) {
        ...
        return Object.assign({}, student, {
            ...
            yearToken: getYearToken(student.year)
        });
    }
